...
> **Nota sobre o escopo deste repositório (2026-08-19):** este repositório contém apenas os contratos **core** (`nexus`, `soul`, `zpay`). Os Spokes de credenciais (`github-identity`, `uber-income`, etc.), verifiers cross-chain (EVM/Cosmos/Solana) e adapters de interoperabilidade (Axelar/LayerZero) — referenciados nas seções abaixo como parte da arquitetura ideal — foram extraídos para [`devfelipenunes/zolvency-interop`](https://github.com/devfelipenunes/zolvency-interop). As seções sobre _Cross-Chain Portability_ (6.2) e _Adapters_ descrevem o design da arquitetura completa; a implementação correspondente vive no repositório de interop.
...
**Invariante operacional:** _sem Soul, sem credencial_.
