...
echo "1. Deploying/Initializing Adapter (NON-OPTIMIZED)..."
# Using the non-optimized WASM
SOUL_ID=$($STELLAR_CLI contract deploy --wasm target/wasm32-unknown-unknown/release/zolvency_soul.wasm --source "$DEPLOYER_SECRET" --network testnet)
ADAPTER_ID=$($STELLAR_CLI contract deploy --wasm target/wasm32-unknown-unknown/release/zolvency_axelar_adapter.wasm --source "$DEPLOYER_SECRET" --network testnet)
...
echo "✅ Direct Will Auth (No Opt) Triggered Successfully!"
