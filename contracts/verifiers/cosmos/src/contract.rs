...
pub fn execute_axelar_message(
    ...
    if payload_bytes[0] == 1 {
        ...
        let soul_id = u32::deserialize(&mut data).map_err(|_| ContractError::InvalidSoulID {})?;
        let user_addr_bytes = <[u8; 32]>::deserialize(&mut data).map_err(|_| ContractError::InvalidAddress {})?;
        let external_id_hash = <[u8; 32]>::deserialize(&mut data).map_err(|_| ContractError::InvalidPayload {})?;
        let tier = u32::deserialize(&mut data).map_err(|_| ContractError::InvalidTier {})?;
        let nonce = u64::deserialize(&mut data).map_err(|_| ContractError::InvalidNonce {})?;
        let token_type_hash = <[u8; 32]>::deserialize(&mut data).map_err(|_| ContractError::InvalidTokenType {})?;
    ...
    if payload_bytes[0] == 2 {
        ...
        let soul_id = u32::deserialize(&mut data).map_err(|_| ContractError::InvalidSoulID {})?;
        let will_address_bytes = <[u8; 32]>::deserialize(&mut data).map_err(|_| ContractError::InvalidAddress {})?;
        let permissions = u64::deserialize(&mut data).map_err(|_| ContractError::InvalidPermissions {})?;
        let expiry = u64::deserialize(&mut data).map_err(|_| ContractError::InvalidExpiry {})?;
